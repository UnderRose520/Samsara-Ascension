...
REPORT = ROOT / "output" / "visual_qa" / "reward_cards_1920_report.txt"
SCREENSHOTS = [
    ROOT / "output" / "visual_qa" / "reward_cards_1920.png",
    ROOT / "output" / "visual_qa" / "reward_full_slot_actions_1920.png",
]
LOG_DIR = ROOT / "tmp"
...
def main() -> int:
    ...
    log = LOG_DIR / f"qa_reward_cards_1920_{os.getpid()}_{int(time.time())}.log"
    ...
    command = [
        ...
        "game",
        "--scene",
        "res://tools/qa_reward_cards_1920.tscn",
        "--log-file",
        f"../tmp/{log.name}",
    ]
    try:
    ...
    if not REPORT.exists():
        ...
        if "Reward card visual QA passed" not in report_text or "Exit code: 0" not in report_text:
            errors.append("Godot reward card visual QA report did not pass")
